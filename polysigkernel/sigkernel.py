...
class SigKernel:
    ...
    def __init__(self,
        ...
        self._solver = self._solver_cls(static_ker=static_kernel, scale=scale, order=order)
    ...
    def _prepare_inputs(
        ...
        if scale == self.scale:
            ...
            solver = self._solver_cls(static_ker=self.static_kernel, scale=scale, order=self.order)
    ...
    def _cross_kernel_sum(
        ...
        for x_slice in self._batch_slices(X.shape[0], max_batch):
            ...
            for y_slice in self._batch_slices(Y.shape[0], max_batch):
                total = total + jnp.sum(solver.solve(Xi, Y[y_slice], False, self.multi_gpu))
    ...
    def kernel_matrix(self,
        ...
        for i, x_slice in enumerate(x_slices):
            for j, y_slice in enumerate(y_slices):
                ...
                blocks[i][j] = solver.solve(X[x_slice], Y[y_slice], block_sym, self.multi_gpu)
    ...
    def compute_distance(self, X, Y, max_batch=100):
        ...
        if X.shape[0] != Y.shape[0]:
            raise ValueError("compute_distance requires X and Y to have the same batch size.")
        ...
        return jnp.diag(K_XX) + jnp.diag(K_YY) - 2. * jnp.diag(K_XY)
    ...
    def compute_scoring_rule(self, X: jax.Array, y: jax.Array, max_batch: int = 100) -> jax.Array:
        ...
        K_XX_m = (K_XX_sum - K_XX_diag_sum) / (X.shape[0] * (X.shape[0] - 1.))
        K_Xy_m = K_Xy_sum / (X.shape[0] * y.shape[0])

        return K_XX_m - 2. * K_Xy_m
    ...
    def compute_expected_scoring_rule(self, X: jax.Array, Y: jax.Array, max_batch: int = 100) -> jax.Array:
        ...
        """
        if X.shape[0] < 2:
            raise ValueError("compute_expected_scoring_rule requires X batch size >= 2.")
    ...
    def compute_mmd(self, X: jax.Array, Y: jax.Array, max_batch: int = 100) -> jax.Array:
        ...
        K_XX_m = (K_XX_sum - K_XX_diag_sum) / (X.shape[0] * (X.shape[0] - 1.))
        K_YY_m = (K_YY_sum - K_YY_diag_sum) / (Y.shape[0] * (Y.shape[0] - 1.))
        K_XY_m = K_XY_sum / (X.shape[0] * Y.shape[0])
...
def hypothesis_test(y_pred: jax.Array,
    ...
    rejected = bool(TU > c)
    if rejected:
        print(f'Hypothesis rejected: distributions are not equal with {confidence_level*100}% confidence')
    else:
        print(f'Hypothesis accepted: distributions are equal with {confidence_level*100}% confidence')
    return rejected
