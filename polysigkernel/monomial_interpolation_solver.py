...
class MonomialInterpolationSolver:
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _static_kernel_diag_update(
        self, p: int, diag_axis_masked: jnp.ndarray, X: jnp.ndarray, Y: jnp.ndarray
    ) -> jnp.ndarray:

        return jax.vmap(
            jax.vmap(
                jax.vmap(
                    lambda i, j, k: jnp.where(
                        k != -1,
                        self.static_kernel(
    ...
    def _get_interpolation_coeffs(
        self, diag_solution_minus1: jnp.ndarray, diag_axis_mask: jnp.ndarray
    ) -> jnp.ndarray:
            ...
            jax.vmap(
                jax.vmap(
                    lambda i, j, k: jnp.where(
                        ...
                        jnp.flip(
                            jnp.polyfit(
                                self.nodes, diag_solution_minus1[i, j, k], self.deg
                            )
                        ),
                        empty,
                    ),
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _get_hyp0f1(
        self, data_at_nodes: jnp.ndarray, diag_axis_mask: jnp.ndarray
    ) -> jnp.ndarray:
        ...
        return jax.vmap(
            ...
            jnp.arange(data_at_nodes.shape[0]),
            jnp.arange(data_at_nodes.shape[1]),
            diag_axis_mask,
        )

    @partial(jax.jit, static_argnums=(0,))
    def _get_data_and_coeffs(
        self,
        p: int,
    ...
    def _solve(self, X: jnp.ndarray, Y: jnp.ndarray) -> jnp.ndarray:
        ...
        diag_solution_minus1 = jnp.zeros(
            shape=(batch_X, batch_Y, diag_length, self.deg + 1), dtype=X.dtype
        )
        diag_solution_minus1 = diag_solution_minus1.at[..., :].set(1.0)

        diag_axis_mask_solution, _ = diag_axis_masks(
            0, length_X, length_Y
        )  # You can brute force the first step and

        def _loop(p, carry):

            diag_solution_minus1, diag_axis_mask_solution = carry
            diag_axis_mask_solution_new, diag_axis_mask_data = diag_axis_masks(
                p, length_X, length_Y
            )

            coeff, _hyp0f1, _i0term = self._get_data_and_coeffs(
                p,
                diag_axis_mask_data,
        ...
        diag_solutions, _ = jax.lax.fori_loop(
            ...
            diag_iterations + 1,
            _loop,
            (diag_solution_minus1, diag_axis_mask_solution),
        )
