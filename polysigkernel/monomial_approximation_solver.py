...
class MonomialApproximationSolver:
    ...
    def __init__(
        ...
        self._static_ker = static_ker
        self._scale = float(scale)
        self._use_linear_fastpath = bool(use_linear_fastpath)
        ...
        def _get_kernel(i: int, j: int, k: int):
            if sym:
                return jnp.where(
                    (k != -1) & (i >= j),
                    self.static_kernel(
                        X[i, k + 1], X[i, k], Y[j, p - k], Y[j, p - k - 1]
                    ),
                    0.0,
                )
            else:
    ...
    def _get_diag_data_linear(
        ...
        """
        Fast path for linear static kernel.

        Computes diag_data[i,j,k] = scale^2 * <dX[i, k], dY[j, p-k-1]> for valid k,
        and 0 for invalid k (k == -1), matching linear_kernel on increments.

        Args:
            p: diagonal index in the PDE loop.
            diag_axis_masked: shape (length_X-1,) with valid k indices or -1.
            dX: X increments, shape (batch_X, length_X-1, dim)
            dY: Y increments, shape (batch_Y, length_Y-1, dim)
            sym: if True, zero out entries where i < j (for symmetric solves).
        """
        k_idx = diag_axis_masked
        k_safe = jnp.maximum(k_idx, 0)

        # Gather X increments at k
        x_inc = jnp.take(dX, k_safe, axis=1, mode="clip")  # (batch_X, K, dim)

        # Corresponding Y increment index is (p-k-1)
        y_idx = p - k_safe - 1
        y_inc = jnp.take(dY, y_idx, axis=1, mode="clip")  # (batch_Y, K, dim)

        s2 = jnp.array(self._scale * self._scale, dtype=dX.dtype)
        dots = s2 * jnp.einsum("ikd,jkd->ijk", x_inc, y_inc)  # (batch_X, batch_Y, K)

        valid_k = (k_idx != -1)[None, None, :]
        dots = dots * valid_k.astype(dots.dtype)

        if sym:
            i_idx = jnp.arange(dX.shape[0])[:, None]
            j_idx = jnp.arange(dY.shape[0])[None, :]
            sym_mask = (i_idx >= j_idx)[:, :, None]
            dots = dots * sym_mask.astype(dots.dtype)
    ...
    def solve(
        ...
        solve_pmap = lambda x, y: self._solve(x, y, sym, False)
